...
# - Job profile JSON files from shared_outputs/job_profile/
...
# - Display of active job profile
# - Display of bias audit summary and chart
#
# Writes:
...
def load_bias_report() -> Dict[str, Any]:
    """
    Loads the bias report if it exists.
    Returns a fallback object if the bias audit has not been run yet.
    """
...
def load_job_profile_for_display() -> Dict[str, Any]:
    """
    Loads the active job profile created by the job description analyser.
    """
    ...
    if not job_files:
        return {
            "available": False,
            "job_id": "unknown",
            "title": "No job profile found",
            "requirements": [],
            "experience_years_required": 0,
            "education_required": "unknown",
            "message": (
                "No job profile JSON was found. Run "
                "task2_job_analyser/job_analyser.py first."
            )
        }

    selected_file = job_files[0]

    if len(job_files) > 1:
        print(
            "Multiple job profile files found. "
            f"Using the first one alphabetically: {selected_file.name}"
        )

    job_profile = load_json(selected_file)
    job_profile["available"] = True
    job_profile["source_file"] = selected_file.name
...
def combine_candidate_records(
    ...
    """
    Combines parsed CV data and scored candidate data into one searchable record.
    """
    combined_records = []
...
def index():
    """
    Main web page route.
    Displays the recruiter search interface, candidate results, job profile, and bias audit.
    """
    load_api_key()
    client = OpenAI()
    ...
    job_profile = load_job_profile_for_display()
...
def bias_chart():
    """
    Serves the bias chart image if it exists.
    """
